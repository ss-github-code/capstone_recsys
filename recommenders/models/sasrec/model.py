...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, attention_dim, num_heads, dropout_rate):
        ...
        self.attention_dim = attention_dim                        # 100
...
class LayerNormalization(tf.keras.layers.Layer):
    ...
    Used to normalize the inputs across features (i.e. 0 mean, variance=1)
    Benefits: stabilize and accelerate NN training.
    Unlike Batch normalization, stats used here are independent of other samples in the same batch.
    """
    ...
    def __init__(self, seq_max_len, embedding_dim, epsilon):
        ...
        self.seq_max_len = seq_max_len
        self.embedding_dim = embedding_dim
        self.epsilon = epsilon  # 1e-8
        self.params_shape = (self.seq_max_len, self.embedding_dim)  # 50, 100
    ...
    def call(self, x):
        """Model forward pass.
        LayerNorm(x) = gamma element-wise product ((x - mean)/sqrt(var + epsilon) + beta
            Element-wise product == Hadamard product
            mean, variance of x; gamma and beta are learned scaling factor and bias
        Args:
            x (tf.Tensor): Input tensor.
...
class SASREC(tf.keras.Model):
    ...
    def __init__(self, **kwargs):
        ...
        self.seq_max_len = kwargs.get("seq_max_len", 100)                # 50
        self.num_blocks = kwargs.get("num_blocks", 2)                    # 2
        self.embedding_dim = kwargs.get("embedding_dim", 100)            # 100	
        self.attention_dim = kwargs.get("attention_dim", 100)            # 100
        self.attention_num_heads = kwargs.get("attention_num_heads", 1)  # 1
        self.conv_dims = kwargs.get("conv_dims", [100, 100])
        self.dropout_rate = kwargs.get("dropout_rate", 0.5)
        self.l2_reg = kwargs.get("l2_reg", 0.0)                          # 0.0
        self.num_neg_test = kwargs.get("num_neg_test", 100)              # 100
        self.chkpoint_dirpath = kwargs.get("chkpoint_dirpath", None)

        print("Dropout rate", self.dropout_rate, "L2 reg", self.l2_reg)

        self.item_embedding_layer = tf.keras.layers.Embedding(
            self.item_num + 1,   # input dim
    ...
    def call(self, x, training):
        ...
        # add positional embeddings, self-attention doesn't include any recurrent or convolutional module
    ...
    def predict(self, inputs):
        ...
        mask = tf.expand_dims(tf.cast(tf.not_equal(input_seq, 0), tf.float32), -1)
        seq_embeddings, positional_embeddings = self.embedding(input_seq)
        # print(input_seq.shape, seq_embeddings.shape, positional_embeddings.shape)
        seq_embeddings += positional_embeddings
        # seq_embeddings = self.dropout_layer(seq_embeddings)
        seq_embeddings *= mask
        seq_attention = seq_embeddings
        ...
        # print("test_logits", test_logits.shape, tf.shape(input_seq)[0])
        # print("seq_emb", seq_emb.shape, candidate_emb.shape)
        # (200, 100) * (1, 101, 100)
    ...
    def train(self, dataset, sampler, valid_sampler, **kwargs):
        """
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % val_epoch == 0:
                ...
                t_valid = self.evaluate_valid_sampler(dataset, valid_sampler, batch_size)
                #t_valid = self.evaluate_valid(dataset)
                print(
                    f"\nepoch: {epoch}, time: {T}, valid (NDCG@10: {t_valid[0]}, HR@10: {t_valid[1]})"
                )
                self.valid0_score.append(t_valid[0])
                self.valid1_score.append(t_valid[1])
                # print(
                #     f"epoch: {epoch}, time: {T},  test (NDCG@10: {t_test[0]}, HR@10: {t_test[1]})"
                # )
                t0.start()

            t_loss_np = tf.math.reduce_mean(step_loss).numpy()
            print("training loss", round(t_loss_np, 4))
            self.train_loss.append(round(t_loss_np, 4))
            if self.chkpoint_dirpath is not None:
                self.save_weights(self.chkpoint_dirpath + f'/chkpt_{epoch}')

        t_test = self.evaluate_valid_sampler(dataset, valid_sampler, batch_size)
        print(f"\nepoch: {epoch}, test (NDCG@10: {round(t_test[0], 4)}, HR@10: {round(t_test[1], 4)})")
    ...
    def evaluate(self, dataset):
        ...
        for u in tqdm(users, ncols=70, leave=False, unit="b"):
            ...
            # with the ground truth item. Based on the ranking of these 101 items, Hit@10 and NDCG@10 is calculated.
    ...
    def evaluate_valid(self, dataset):
        ...
        valid_loss = []
        # v_loss = []
        labels = np.zeros((1, 1 + self.num_neg_test), dtype=np.float32)
        labels[0][0] = 1.0
        ...
        for u in tqdm(users, ncols=70, leave=False, unit="b"):
            ...
            predictions = -1.0 * self.predict(inputs)
            predictions = np.array(predictions)
            valid_loss.append(bce(labels, predictions).numpy())
            # res = cal_metric(labels.flatten(), tf.nn.sigmoid(predictions).numpy().flatten(), ['logloss'])
            # v_loss.append(res['logloss'])
            # print(valid_loss[-1], v_loss[-1])
            predictions = predictions[0]
        ...
        print("valid loss", np.mean(valid_loss), valid_user)#, np.mean(v_loss))
        self.valid_loss.append(np.mean(valid_loss))
        return NDCG / valid_user, HT / valid_user

    def evaluate_valid_sampler(self, dataset, valid_sampler, batch_size):
        """
        Evaluation on the validation users
        """
        NDCG = 0.0
        valid_user = 0.0
        HT = 0.0
        num_steps = int(len(dataset.user_valid) / batch_size)

        labels = np.zeros((batch_size, 1 + self.num_neg_test), dtype=np.float32)
        labels[:, 0] = 1.0
        valid_loss = []
        bce = tf.keras.losses.BinaryCrossentropy(from_logits=True)
        for step in tqdm(range(num_steps), total=num_steps, ncols=70, leave=False, unit="b"):
            u, seq, item_idx = valid_sampler.next_batch()
            ...
            valid_loss.append(bce(labels, predictions).numpy())
            # res = cal_metric(labels.flatten(), tf.nn.sigmoid(predictions).numpy().flatten(), ['logloss'])
            # print(valid_loss[-1], res['logloss'])
            for i in range(batch_size):
                ...
                if rank < 10:
                    NDCG += 1 / np.log2(rank + 2)
                    HT += 1
        vloss_ = round(np.mean(valid_loss),4)
        print("valid loss", vloss_, int(valid_user))#, np.mean(v_loss))
        self.valid_loss.append(vloss_)
        return NDCG / valid_user, HT / valid_user

    def evaluate_test_sampler(self, dataset, test_sampler, batch_size):
        """
        Evaluation on the validation users
        """
        NDCG = 0.0
        valid_user = 0.0
        HT = 0.0
        num_steps = int(len(dataset.user_test) / batch_size)

        labels = np.zeros((batch_size, 1 + self.num_neg_test), dtype=np.float32)
        labels[:, 0] = 1.0

        for step in tqdm(range(num_steps), total=num_steps, ncols=70, leave=False, unit="b"):
            u, seq, item_idx = test_sampler.next_batch()
